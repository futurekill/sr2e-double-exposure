#!/bin/zsh
# Generate actor portraits for the Double Exposure cast via the Codex imagegen
# skill. Resumable (skips portraits that already exist). Run from the module
# root:  zsh tools/gen-portraits.sh
# Requires Codex CLI logged in with image-generation quota available.
set -u
MODULE=/Users/jcandalino/Code/foundryvtt/shadowrun/sr2e-double-exposure
...
WORK=${TMPDIR:-/tmp}/de-portraits
mkdir -p $WORK "$MODULE/assets/portraits"
cd $MODULE || exit 1

PREAMBLE='Use your imagegen skill with the built-in image_gen tool (NOT the CLI fallback). The FOUR attached images are STYLE REFERENCES ONLY for the painterly RENDERING and dark cinematic mood — gritty Shadowrun cyberpunk-fantasy, highly detailed. These are ACTOR PORTRAITS for a Shadowrun adventure. Each item is tagged PERSON (a character portrait, head-and-shoulders to waist-up, a real individual with personality) or CREATURE (a full creature/monster portrait). Perfect SQUARE 1:1 (1024x1024), the subject centered and comfortably inside the frame with a little margin so nothing important clips (these become battlemap tokens). CRUCIAL: VARY the palette, lighting and mood per subject exactly as its MOOD tag says — do NOT default to purple/violet, that colour is overused; pull from the full range (steel, teal, amber, green, red, white, brown, gold). Generate a SEPARATE new square image for EACH subject and SAVE it as webp to the EXACT path given. If a path already exists, SKIP it. Report each saved path.'
